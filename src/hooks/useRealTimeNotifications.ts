...
export const useRealTimeNotifications = (guardianId: string) => {
  ...
  const handleNewNotification = useCallback(async (notification: RealTimeNotification) => {
    ...
    try {
      console.log('🔊 useRealTimeNotifications: Reproduzindo som para:', notification.type);
      await audioService.playNotificationSound(notification.type as any);
      console.log('✅ useRealTimeNotifications: Som reproduzido com sucesso');
    } catch (error) {
      console.warn('❌ useRealTimeNotifications: Não foi possível tocar som de notificação:', error);
    }
